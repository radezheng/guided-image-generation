...
if 'loaded' not in st.session_state:
    ...
    if access_code != correct_access_code:
        st.write('Incorrect access code. Please try again.')
        st.stop()
...
with st.sidebar:
    ...
    llm_deployment = st.selectbox("输入优化提示的模型:", 
    ...
    if with_stable_diffusion == "True":
        imgen_deployment = st.selectbox("文生图的模型:",
    ...
    image_size = st.selectbox("图片大小:", sizes, index=1, help="Image resolution width x height")
    imgen_style = st.selectbox("生成风格(优化提示才生效):", styles, index=17,)
    ...
    gpt4v = bd_col1.toggle('GPT4-Vision', False)
    azure_image_analysis = bd_col2.toggle('Azure Image Analysis', False)

    # Severity level of image moderation    
    cs_severity2level = {'safe' : 0, 'low' : 2, 'medium' : 4, 'high' : 6}
    cs_threshold = cs_severity2level[st.select_slider("Image Moderation Threshold", cs_severity2level.keys(), 'high', help='Images with the selected severity level and higher will be rejected.')]
...
user_prompt = st.chat_input("描述您的图片")
...
if user_prompt:
    ...
    for entry in st.session_state.history:
            ...
            if entry['refined_prompt'] != entry['prompt']:
                st.write('Refined:', entry['refined_prompt'])
            st.image(entry['image'], caption=entry['caption'])
    ...
    st.write('prompt inputed: ', user_prompt)
    if llm_deployment == 'None':
        ...
        try:
            with st.spinner('Refining Prompt ...'):
                ...
                st.write('refined_prompt: ', refined_prompt)
        except Exception as e:
    ...
    new_entry = {'prompt' : user_prompt, 'refined_prompt' : refined_prompt, 'image' : image, 'caption' : caption}
